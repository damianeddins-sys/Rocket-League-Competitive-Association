...
export default async function FranchisesPage({
  ...
  const [franchises, snapshot] = await Promise.all([
    getFranchises(),
  ...
  return (
    <div className="min-h-screen">
      <PageHero eyebrow="League organizations" title="Franchises" description="Franchise destinations connect front-office identity, associated teams, rosters, and public league history."><SeasonSelector seasons={snapshot.seasons} current={snapshot.season} pathname="/franchises" /></PageHero>
        ...
        {franchises.length === 0 ? <EmptyState title="No franchises published" message="Official franchise records will appear here after league operations publishes them." /> : (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {franchises.map((franchise) => {
              ...
              return <Link key={franchise.id} href={`/franchises/${franchise.slug}${seasonQuery}`} className="entity-card group p-6">
