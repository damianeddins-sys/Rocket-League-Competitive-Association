...
import { RULEBOOK_SECTIONS } from "./rulebook";
...
describe("official RLCA 2v2 season format", () => {
  it("publishes the specified regular-season weeks and BO5 schedule", () => {
    expect(ruleText["Regular season — Weeks 1–4"]).toEqual(
      [1, 2, 3, 4].map(
        (week) =>
          `Week ${week}: One hour of scrimmages, followed by two normal league matches. Each match is Best of 5.`,
      ),
    );
  ...
  it("keeps every event's qualification and seeding rules explicit", () => {
    ...
    expect(ruleText["Championship Major"]).toEqual(expect.arrayContaining([
      ...
      expect.stringContaining("ranked #1 and #2 immediately before the Last Chance Major retain those Championship Major seeds"),
      expect.stringContaining("applicable record and points rules"),
    ]));
  });
  ...
  it("uses seeding terminology without inventing a points calculation", () => {
    ...
    expect(ruleText["Qualification Points"]).toEqual([
